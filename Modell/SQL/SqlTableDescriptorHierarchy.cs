...
namespace TestToolsBase.Modell
{
    ...
    public abstract class SqlTableDescriptorHierarhyTableBase : SqlTableDescriptorBase
    {
        ...
        /// TestToolsBase loads this records to new object, with '_sql' postfix at ColectionKey 
        /// (because we cannot sure that counted value is exist, so we check the 2 collection )
        ///  TestToolsBase SqlHiLevel.LoadAll() function will append collectionKey by 
    ...
    public abstract class SqlTableDescriptorHierarhyTableChild_Internal : SqlTableDescriptorHierarhyTableBase
    {
        ...
        {
            throw new TestToolsBaseInternalException("use 'CreateStorableClass(NpgsqlDataReader dr_in)'");
        }
