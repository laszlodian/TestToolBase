...
namespace TestToolsBase.Modell
{
    ...
    public class MeasureRoot : INotifyCollectionChanged
    {
        ...
        public virtual void Database_Add(string collectionKey_in, object obj_in)
        {
            ...
            {
                if (this is TestToolsBaseClass)
                    MeasureCollectionBase.TheMeasures.Measures.Add((obj_in as IMyMeasure).MyMeasure = (this as TestToolsBaseClass));
                else
                    throw new InvalidOperationException("IMyMeasure interface can contains TestToolsBase, not MeasureCollectionBase. There is no critical reason for it, only I thought TestToolsBase is better/more usable. If you use this class at database of MeassureCollection, you can reach the containing database class by 'MeassureCollectionBase.TheMeassures'.");
            }
                ...
                using (NpgsqlCommand tmpBuffer = new NpgsqlCommand())//storage for runtime counted values
                {
                    foreach (object item in items)
                    {
                        ...
                        foreach (string resultColumn in resultColumns)
                        {
                            ...
                            {
                                throw new TestToolsBaseInternalException(string.Format("ResultColumn {0} cannot be founeded at returned parameters of {1}.SqlSave ",
