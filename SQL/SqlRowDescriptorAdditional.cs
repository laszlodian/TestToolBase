    ...
    public class SqlRowDescriptorAdditionalLeaf : SqlRowDescriptorAdditional
    {
                        ...
                        SqlLowLevel.CreateWhereClause(data)), connection_in))
                {
                    ...
                    using (NpgsqlDataReader dr = query.ExecuteReader())
                    {
                        if (dr.HasRows)
                        {
                            ...
                            if (dr.Read())
                            {
                                throw new TestToolsBaseInternalException("There is MORE record for {0}", this);
                            }
                        }
                        dr.Close();
                    }                        
        ...
        internal void SqlLoadInternal(NpgsqlConnection sqlConn_in)
        {
            ...
            if (this.RowId == SqlLowLevel.INVALID_ROW_ID)
            {
                ...
                throw new TestToolsBaseInternalException("RowId must be set before calling this function. (It is stored in parent table fk_{0}_id )", this.TableDescriptor.TableName);
            }
                ...
                this.TableDescriptor.TableName, SqlLowLevel.COLUMN_NAME_ID, this.RowId), sqlConn_in))
            {
                using (NpgsqlDataReader dr = query.ExecuteReader())
                {
                    if (dr.HasRows)
                    {
                        ...
                        throw new TestToolsBaseInternalException("There is no stored record for {0}.", this);
                }
            }//using query
        }
        ...
        internal void CheckConsistency()
        {
            ...
            foreach(KeyValuePair<string, object> loaded in SqlLoadedResults)
            {
                ...
                if (current.ContainsKey(loaded.Key))
                {
                    ...
                    throw new TestToolsBaseInternalException(string.Format("ResultColumn {0} cannot be founeded at returned parameters of {1}.GetData ",
                                    loaded.Key, this));
            }
        }
