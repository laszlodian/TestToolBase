...
namespace TestToolsBase
{
    ...
    public ResultForm(string lot, string roll, string meas_type, double[] glu, double[] nano_amp, int[] sn, string[] bar, bool[] remeas)
    {
        InitializeComponent();
        ...
        foreach (double glucose in glu)
        {
            bindingRollSource.Add(
                new ResultModell(lot, roll, meas_type, glu[i], nano_amp[i], sn[i], bar[i], remeas[i]));
            i++;
        }
    ...
    public ResultForm(string lot, string[] roll, double[] blank_averages, bool[] blank_is_valid,
        double[] blank_ceve,
         DateTime[] test_date,
        int[] tube_count,
        double[] blank_stddev)
