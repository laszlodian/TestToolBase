...
using System.Linq;
using System.Text;
using TestToolsBase.Configuration;
...
namespace TestToolsBase.Configuration
{
	/// <summary>
	...
	public class MeasureConfig : ISqlAdditionalObj
	{
		...
		public virtual void Init(System.Configuration.ApplicationSettingsBase[] config_in)
        {
            ...
            if ((GetConfigValueOf("DBSelector").ToString() == "UD2"))
            {
                ...
                if (EnvironmentId.ForceTestDb || !Convert.ToBoolean(GetConfigValueOf(EDefaultConfigs.DBUseRelease.ToString())))
                {
                    ...
                    SqlConnectionStr = TestToolsBase.Sql.SqlLowLevel.DecryptPw((string)AppConfigMain[EDefaultConfigs.UD2DebugConnectionString.ToString()]);
                    ...
                    if (EnvironmentId.TheEnvironmentId.IsConsole_nGui)
                    {
                        ...
                        System.Console.BackgroundColor = y; System.Console.Write("              "); System.Console.BackgroundColor = b; System.Console.WriteLine();
                        System.Console.BackgroundColor = y; System.Console.Write("   ");
                ...
                {
                    SqlConnectionStr = TestToolsBase.Sql.SqlLowLevel.DecryptPw((string)AppConfigMain[EDefaultConfigs.UD2ReleaseConnectionString.ToString()]);
                    SqlIsReleaseDb = true;
            ...
            {        
                ...
                if (EnvironmentId.ForceTestDb || !Convert.ToBoolean(GetConfigValueOf(EDefaultConfigs.DBUseRelease.ToString())))
                {
                    ...
                    SqlConnectionStr = TestToolsBase.Sql.SqlLowLevel.DecryptPw((string)AppConfigMain[EDefaultConfigs.LFRDebugConnectionString.ToString()]);
                    SqlIsReleaseDb = false;
                    Trace.TraceInformation("Test DB");
                    Trace.TraceInformation("SQL connection: {0}", SqlLowLevel.RemovePasswordFromSqlConnectionString(SqlConnectionStr));
                    WinFormTestDbControl = new TestDbControl();
                ...
                {
                    SqlConnectionStr =TestToolsBase.Sql.SqlLowLevel.DecryptPw((string)AppConfigMain[EDefaultConfigs.LFRReleaseConnectionString.ToString()]);
                    SqlIsReleaseDb = true;
                    Trace.TraceInformation("Release DB");
                    Trace.TraceInformation("SQL connection: {0}", SqlLowLevel.RemovePasswordFromSqlConnectionString(SqlConnectionStr));
                    WinFormTestDbControl = null;
            ...
            else if (GetConfigValueOf("DBSelector").ToString() == "UA3")
            {
                ...
                if (EnvironmentId.ForceTestDb || !Convert.ToBoolean(GetConfigValueOf(EDefaultConfigs.DBUseRelease.ToString())))
                {
                    ...
                    SqlConnectionStr = TestToolsBase.Sql.SqlLowLevel.DecryptPw((string)AppConfigMain[EDefaultConfigs.UA3DebugConnectionString.ToString()]);
                    SqlIsReleaseDb = false;
                    Trace.TraceInformation("Test DB");
                    Trace.TraceInformation("SQL connection: {0}", SqlLowLevel.RemovePasswordFromSqlConnectionString(SqlConnectionStr));
                    WinFormTestDbControl = new TestDbControl();
                ...
                {
                    SqlConnectionStr = TestToolsBase.Sql.SqlLowLevel.DecryptPw((string)AppConfigMain[EDefaultConfigs.UA3ReleaseConnectionString.ToString()]);
                    SqlIsReleaseDb = true;
                    Trace.TraceInformation("Release DB");
                    Trace.TraceInformation("SQL connection: {0}", SqlLowLevel.RemovePasswordFromSqlConnectionString(SqlConnectionStr));
                    WinFormTestDbControl = null;
		...
		public void InitSql(bool releaseDb_in, string releaseDbConnection, string debugDbConnection)
		{
			...
			{
				Trace.TraceInformation("Release DB");
				SqlConnectionStr = TestToolsBase.Sql.SqlLowLevel.DecryptPw(releaseDbConnection);
			}
			else
			{
				Trace.TraceInformation("Test DB");
				SqlConnectionStr = TestToolsBase.Sql.SqlLowLevel.DecryptPw(debugDbConnection);
			}
		}
		...
		public object GetIniFileConfig(string iniParamDescriptor_in)
		{
			...
			if (!IniFileConfigs.ContainsKey(GetConfigId(iniParamDescriptor_in)))
			{
				...
				{ //load ini file
					...
					if (System.IO.File.Exists(_iniFName))
					{
						iniList.Clear();
						try
						{
